#ifndef MARKOVCHAINS_HH
#define MARKOVCHAINS_HH
...
public:
  ...
  // Excludes seqID from building model; used for leave-one-out validation
  MarkovChains_t( int order,
                   const char *seqID, // seq ID of a sequence to be excluded from model building
...
//-------------------- inlines -------------------------------
inline const vector<char *> & MarkovChains_t::modelIds()
{ return modelIds_m; }

inline void MarkovChains_t::modelIds( vector<string> &v )
{
...
inline const vector<vector<char *> > & MarkovChains_t::wordStrgs() const
{ return wordStrgs_m; }

inline int MarkovChains_t::hashFnUL( int L )
{
    int h = 0;
...
inline int MarkovChains_t::hashFn(const char *s, int sLen)
{
    //int h = hashFnUL(sLen);
...
inline int MarkovChains_t::hashFn(const char c)
{
    return intACGTLookup[int(c)];
}

inline void MarkovChains_t::printCounts()
{
    printCounts_m = true;
}

inline double MarkovChains_t::normLog10prob( const char *frag, int fragLen, int modelIdx )
{
    return log10prob( frag, fragLen, modelIdx ) / fragLen;
}

inline double MarkovChains_t::normLog10probIUPAC( const char *frag, int fragLen, int modelIdx )
{
    return log10probIUPAC( frag, fragLen, modelIdx ) / fragLen;
}

inline double MarkovChains_t::normLog10probR( char *frag, int fragLen, int modelIdx )
{
    return log10probR( frag, fragLen, modelIdx ) / fragLen;
}

inline int MarkovChains_t::order()
{
    return order_m;
}
