...
void printUsage( const char *s )
{
  cout << endl
       ...
       << "USAGE " << endl
       << endl
       << "  Estimating log10 posterior probabilities of random sequences from all taxons (except the domain) models. The random sequences are from the model and the model's siblings.\n"
       << endl
...
int main(int argc, char **argv)
{
    ...
    if ( inPar->verbose )
      fprintf(stderr,"--- Reading k-mer frequency tables from %s\n", inPar->mcDir);
    ...
    while ( !bfs.empty() )
    {
      ...
      if ( node != root )
      {
        ...
        // node->idx >= 0 <=> numChildren==0 <=> the node is a leaf, that is a species
        //if ( node->idx >= 0 ) // processing all taxonomic ranks
        {
          if ( inPar->verbose )
