/*
  Use multiple threads to estimate error thresholds only at the species level
  using a criterion that no more than, thld, percent of reference sequences
  should be below the threshold value. Initially the threshold is set to the max
  of pp of sibling's ref seq's w/r to the reference species model. If the
  percentage of ref species' seq's below this max is more than thld, then set
  the threshold at the thld-percentile.
...
void processNode( queue<NewickNode_t *>  &bfs, inPar_t *inPar )
{
  mtx.lock();
  bool isempty = bfs.empty();
  mtx.unlock();
