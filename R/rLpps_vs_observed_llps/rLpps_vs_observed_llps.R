...
file1 <- "~/devel/speciateIT/data/vaginal_Mar2021_V3V4/V3V4_vag_mcDir/Lpps/BVAB1.csv"
...
file <- paste0(pics.dir, ref.tx,"_lpps.pdf")
pdf(file, width=7, height=6)
op <- par(mar=c(4, 3.75, 1.75, 0.75), mgp=c(2.25,0.5,0),tcl = -0.3)
r <- plot.dlpp(lpps, ref.tx)
par(op)
dev.off()
...
file <- paste0(pics.dir, ref.tx,"_rlpps.pdf")
pdf(file, width=7, height=6)
op <- par(mar=c(4, 3.75, 1.75, 0.75), mgp=c(2.25,0.5,0),tcl = -0.3)
rr <- plot.drlpp(rlpps, ref.tx, lpps)
par(op)
...
dir <- "~/devel/speciateIT/data/vaginal_Mar2021_V3V4/V3V4_vag_mcDir/Lpps"
...
{
    cat("\r",sp)
    file1 <- paste0("~/devel/speciateIT/data/vaginal_Mar2021_V3V4/V3V4_vag_mcDir/Lpps/",sp,".csv")
    lpps <- read.csv(file1, header=FALSE)
    colnames(lpps) <- c("seqID","tx","lpp")
    ##
    file2 <- paste0("~/devel/speciateIT/data/vaginal_Mar2021_V3V4/V3V4_vag_mcDir/rLpps/",sp,".csv")
