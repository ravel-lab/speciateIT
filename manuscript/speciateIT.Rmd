...
Anything species with > 10 sequences (which includes less-important vaginal species), pick one sequence at random which had not been misclassified. 
...
for (i in c("V1V3", "V3V4", "V4V4")){
  print(i)
  a<-class[class$var %in% i, ]
  for (taxon in unique(a$actual)){
    s<-a[a$actual %in% taxon & a$correct %in% "1", ]
    #t<-a[a$actual %in% taxon & a$correct %in% "0", ]
    if(nrow(s) > 0){ 
      pp<-as.vector(s$V3) ## if all seq's are correct, loosen the threshold a little (reduce by adding log10)
      err.thld<-rbind(err.thld, cbind(Taxon=taxon, Threshold=log10(min(pp))-0.05, var=i))
    }
  }
}
write.table(err.thld[err.thld$var %in% "V1V3", c("Taxon", "Threshold")], "final_models/vSpeciateIT_V1V3/error_thlds.txt", sep="\t", quote=F, row.names = F)
write.table(err.thld[err.thld$var %in% "V3V4", c("Taxon", "Threshold")], "final_models/vSpeciateIT_V3V4/error_thlds.txt", sep="\t", quote=F, row.names = F)
write.table(err.thld[err.thld$var %in% "V4V4", c("Taxon", "Threshold")], "final_models/vSpeciateIT_V4V4/error_thlds.txt", sep="\t", quote=F, row.names = F)
```
...
all.lin %>% group_by(reg) %>% summarise(Sp=length(unique(V1)), G=length(unique(V2)), F=length(unique(V3)), O=length(unique(V4)), C=length(unique(V5)), P=length(unique(V6)))
```

#14. Cross Validation
```{bash}
for f in vSpeciateIT_V*V*.good.unique.keep2.vicut.tx; do ./pecan_cv5.pl -o ${f/.tx}_cv -f ${f/.tx}.fasta -a $f -l ${f/.tx}.lineage -n 10 -s 1; done
```
...
ggplot(known2, aes(x=known, fill=level))+geom_bar(color="black", lwd=0.2)+
...
### speciateIT CLASSIFY
```{bash}
time ./bin/classify -d final_models/vSpeciateIT_V3V4.good.unique.keep2.vicut -i V3V4_indep.good.unique.fasta -o V3V4_ind
time ./bin/classify -d final_models/vSpeciateIT_V1V3.good.unique.keep2.vicut -i V1V3_indep.good.unique.fasta -o V1V3_ind
time ./bin/classify -d final_models/vSpeciateIT_V4V4.good.unique.keep2.vicut -i V4V4_indep.good.unique.fasta -o V4V4_ind
```
...
for f in ASV_files/*taxa.fasta; do rm -rf ${f/_all_runs_dada2_abundance_table_w_taxa.fasta} | bin/classify -d final_models/vSpeciateIT_V3V4 -i $f -o ${f/_all_runs_dada2_abundance_table_w_taxa.fasta}; done
...
### 16a. Hierarchical clustering
```{r}
the.samples<-read.csv("the.samples.csv")
the.prop<-the.samples[,3:ncol(the.samples)]/rowSums(the.samples[,3:ncol(the.samples)])
rownames(the.prop)<-the.samples$sampleID
the.prop<-the.prop[,order(colSums(the.prop), decreasing = TRUE)]

## Distances between samples
the.dist<-vegdist(as.matrix(the.prop), na.rm = T)
the.clsting<-hclust(the.dist, method="ward")

to.plot<-the.prop[,1:50]
...
the.clusts<-as.data.frame(cbind(cluster=cutree(the.clsting, k=9), sampleID=rownames(the.prop)))
the.clusts<-cbind(the.clusts, domTaxa=colnames(the.prop)[max.col(the.prop, ties.method="first")])
the.clusts.sort<-the.clusts %>% group_by(cluster, domTaxa) %>% summarise(n=length(unique(sampleID)))
the.clusts.cols<-cbind(cluster=unique(the.clusts.sort$cluster), domTaxa=the.clusts.sort$domTaxa[max(the.prop, ties.method="first")])
the.clusts$cluster<-as.factor(the.clusts$cluster)
cols<-RColorBrewer::brewer.pal(12, "Set3")[1:9]

names(cols)<-unique(the.clusts$cluster)
## cut for 9, then pull out IVC and recluster to get the other 5
...
pheatmap(t(as.matrix(to.plot)), 
         ...
         annotation_row = NULL,
         cutree_cols = 9, ## at cutree=20, Enterococcus comes out as separate group.
         legend = TRUE, 
         fontsize = 6, 
         labels_col = "", 
...
the.clusts<-as.data.frame(cbind(cluster=cutree(the.clsting, k=9)))
the.clusts<-cbind(the.clusts, domTaxa=colnames(the.prop)[max.col(the.prop, ties.method="first")])
the.clusts$sampleID<-rownames(the.clusts)
the.clusts.sort<-the.clusts %>% group_by(cluster, domTaxa) %>% summarise(n=length(unique(sampleID)))
ivc<-the.clusts.sort$cluster[the.clusts.sort$n == max(the.clusts.sort[the.clusts.sort$domTaxa %in% "g_Bifidobacterium", "n"])]

## IVC
the.samples<-the.samples2[the.samples2$sampleID %in% the.clusts$sampleID[the.clusts$cluster %in% ivc], ]
the.prop<-the.samples[,3:ncol(the.samples)]/rowSums(the.samples[,3:ncol(the.samples)])
rownames(the.prop)<-the.samples$sampleID
the.prop<-the.prop[,order(colSums(the.prop), decreasing = TRUE)]

## Distances between samples
the.dist.ivc<-vegdist(as.matrix(the.prop), na.rm = T)
the.clsting.ivc<-hclust(the.dist.ivc, method="ward")
the.clusts.ivc<-as.data.frame(cbind(cluster=cutree(the.clsting.ivc, k=5)))
the.clusts.ivc$cluster<-as.factor(the.clusts.ivc$cluster)
...
the.clusts$sampleID<-rownames(the.clusts)
the.clusts$CST<-
relabund$CST<-the.tax.sub[match(rownames(relabund), the.tax.sub$sampleID), "CST"]
relabund$sampleID<-rownames(relabund)
relabund$CST<-factor(relabund$CST, levels=c("I-A", "I-B", "II", "III-A", "III-B", "IV-A", "IV-B", "IV-C1", "IV-C2", "IV-C3", "IV-C5", "IV-C6", "V"))
the.data.m<-reshape2::melt(relabund, id.vars = c("sampleID", "CST"), value.name = "relabund", variable.name = "Taxon")
...
write.csv(centroids, paste("VALENCIA2_CST_centroids_", today2, ".csv", sep=""), row.names = F, quote=F)
```

### 16e. Run VALENCIA
```{bash}
python3 ~/bin/VALENCIA/Valencia.py -r ../VALENCIA2_CST_centroids_20Mar2024.csv -i the.samples.csv -o the.samples.cst
```
...
class<-read.csv("the.samples.cst.csv")
class$subCST<-factor(class$subCST, levels=unique(c("I-A", "I-B", "II", "II", "III-A", "III-B", "IV-A", "IV-A", "IV-B", "IV-B", "IV-C1", "IV-C1", "IV-C2", "IV-C2", "IV-C3", "IV-C3", "IV-C4", "IV-C4", "V")))
rownames(class)<-class$sampleID

class.old<-read.csv("the.samples.cst.old.csv")
class.old$subCST<-factor(class.old$subCST, levels=unique(c("I-A", "I-B", "II", "II", "III-A", "III-B", "IV-A", "IV-A", "IV-B", "IV-B", "IV-C0", "IV-C1", "IV-C1", "IV-C2", "IV-C2", "IV-C3", "IV-C3", "IV-C4", "IV-C4", "V")))
class.old<-class.old[class.old$sampleID %in% class$sampleID, ]
names(class.old)[which(colnames(class.old) %in% "subCST")]<-"VALENCIA1"

class.cmp<-merge(class[,c("sampleID", "subCST")], class.old[,c("sampleID", "VALENCIA1")])
names(class.cmp)[2]<-"VALENCIA2"
table(class.cmp$VALENCIA2, class.cmp$VALENCIA1, dnn = c("new", "old"))
#        old
# new      I-A  I-B   II III-A III-B IV-A IV-B IV-C0 IV-C1 IV-C2 IV-C3 IV-C4    V
#   I-A    636  144  213   765   380   22   38   342    28    12     0   220  656
#   I-B      9   77   14   160    70    1    8    63     4     2     1     7   23
#   II       0    2  270    10    16    6    2   113     7     0    55     2    0
#   III-A    4   12    5  3570   232    8    1    12     0     1     0    11    9
#   III-B    0    1    0   538   489   11   28     9     2     0     0     4    0
#   IV-A     0    0    0    14    45  955   33     1     0     0     0     0    0
#   IV-B     1   11   53   162   679  428 1011   249    38     2     3    21   10
#   IV-C0    0    4    4    46    59   46  111   138    48    21    14    37    8
#   IV-C1    0    0    5    23    10    0    2     6   102     0    20     7    1
#   IV-C2    1   10    4     7     4    0    2     8    20     1    35     1    1
#   IV-C3    0    0    6     7     8    0    1     8     0    71     3    13    1
#   IV-C4    0    1    7    35    61  143  171    31     4     0    14     0    0
#   V        1    8   30    59    36    5   37   105     4     0     9     7  107
#
require(irr)
kappa_result <- kappa2(data.frame(class.cmp$VALENCIA1, class.cmp$VALENCIA2), )
```